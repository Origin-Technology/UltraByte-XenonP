...
import dev.ultrabyte.modules.impl.core.RendersModule;
...
import dev.ultrabyte.utils.miscellaneous.RenderPosition;
...
public class RenderManager implements IMinecraft {
    ...
    public void onRenderWorld$placePositions(RenderWorldEvent event) {
        ...
        RendersModule module = UltraByte.MODULE_MANAGER.getModule(RendersModule.class);
    ...
    public void onRenderWorld$autoCrystal(RenderWorldEvent event) {
        ...
        AutoCrystalModule autoCrystalModule = UltraByte.MODULE_MANAGER.getModule(AutoCrystalModule.class);
    ...
    public void onRenderWorld$autoCrystalExtra(RenderWorldEvent.Post event) {
        ...
        AutoCrystalModule module = UltraByte.MODULE_MANAGER.getModule(AutoCrystalModule.class);
        ...
        if (module.icon.getValue()) {
            ...
            if (module.renderDamage.getValue()) {
                Renderer2D.renderTexture(matrices, -5.5f, -8.5f, 5.5f, 2.5f, Identifier.of(UltraByte.MOD_ID, "textures/crystal.png"), Color.WHITE);

                matrices.push();
                matrices.scale(0.45f, 0.45f, 0.45f);

                String text = module.getCalculationDamage();
                UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, text, -UltraByte.FONT_MANAGER.getWidth(text) / 2 - 1, 7, mc.getBufferBuilders().getEntityVertexConsumers(), Color.WHITE);

                matrices.pop();
            } else {
                Renderer2D.renderTexture(matrices, -6.5f, -6.5f, 6.5f, 6.5f, Identifier.of(UltraByte.MOD_ID, "textures/crystal.png"), Color.WHITE);
            }
            ...
            if (module.renderDamage.getValue()) {
                ...
                String text = module.getCalculationDamage();
                UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, text, -UltraByte.FONT_MANAGER.getWidth(text) / 2, -UltraByte.FONT_MANAGER.getHeight() / 2, mc.getBufferBuilders().getEntityVertexConsumers(), Color.WHITE);
    ...
    public void setRenderPosition(BlockPos position) {
        if (!UltraByte.MODULE_MANAGER.getModule(AutoCrystalModule.class).isToggled()) position = null;
