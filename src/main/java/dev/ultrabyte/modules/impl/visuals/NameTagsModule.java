...
import dev.ultrabyte.mixins.accessors.ItemRenderStateAccessor;
import dev.ultrabyte.mixins.accessors.ItemRendererAccessor;
import dev.ultrabyte.mixins.accessors.LayerRenderStateAccessor;
import dev.ultrabyte.modules.Module;
import dev.ultrabyte.modules.RegisterModule;
import dev.ultrabyte.modules.impl.miscellaneous.FakePlayerModule;
...
public class NameTagsModule extends Module {
    ...
    public BooleanSetting ultrabyteCheck = new BooleanSetting("ultrabyteCheck", "Adds an indicator next to the name of other users..", true);
    ...
    public void onRenderWorld(RenderWorldEvent.Post event) {
        ...
        for (PlayerEntity player : mc.world.getPlayers().stream().sorted(Comparator.comparing(p -> -mc.player.distanceTo(p))).toList()) {
            ...
            int width = UltraByte.FONT_MANAGER.getWidth(text);

            if (border.getValue().equalsIgnoreCase("Fill") || border.getValue().equalsIgnoreCase("Both")) Renderer2D.renderQuad(matrices, -width / 2.0f - 1, -UltraByte.FONT_MANAGER.getHeight() - 1, width / 2.0f + 2, 0, fillColor.getColor());
            if (border.getValue().equalsIgnoreCase("Outline") || border.getValue().equalsIgnoreCase("Both")) Renderer2D.renderOutline(matrices, -width / 2.0f - 1, -UltraByte.FONT_MANAGER.getHeight() - 1, width / 2.0f + 2, 0, outlineColor.getColor());

            UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, text, -width / 2, -UltraByte.FONT_MANAGER.getHeight(), vertexConsumers, UltraByte.MODULE_MANAGER.getModule(FakePlayerModule.class).isToggled() && UltraByte.MODULE_MANAGER.getModule(FakePlayerModule.class).getPlayer() == player ? new Color(225, 0, 70) : player.isSneaking() ? new Color(255, 170, 0) : UltraByte.FRIEND_MANAGER.contains(player.getName().getString()) ? UltraByte.FRIEND_MANAGER.getDefaultFriendColor() : Color.WHITE);
            ...
            if (enchantments.getValue()) {
                for (int i = 0; i < 6; i++) {
                    ...
                    if (!component.getEnchantments().isEmpty()) {
                        int height = (component.getEnchantments().size() * UltraByte.FONT_MANAGER.getHeight() / 2) - 18;
            ...
            for (int i = 0; i < 6; i++) {
                ...
                int stackY = -UltraByte.FONT_MANAGER.getHeight() - 1 - (items.getValue() ? 18 + maxEnchants : 1);
                ...
                if (items.getValue()) {
                    ...
                    if (stack.getItem().equals(Items.ENCHANTED_GOLDEN_APPLE)) {
                        ...
                        UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, "God", 0, 0, vertexConsumers, new Color(255, 125, 255));
                        matrices.pop();
                    }
                    ...
                    if (stack.getCount() != 1) {
                        ...
                        matrices.push();
                        matrices.translate(stackX + 17 - UltraByte.FONT_MANAGER.getWidth(count), stackY + 9, 0);
                        UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, count, 0, 0, vertexConsumers, Color.WHITE);
                        matrices.pop();
                    }
                }
                ...
                if (durability.getValue() && stack.isDamageable()) {
                    ...
                    matrices.push();
                    matrices.translate(stackX, stackY - UltraByte.FONT_MANAGER.getHeight() / 2f - 1, 0);
                    matrices.scale(0.5f, 0.5f, 1);
                    UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, Math.round(((stack.getMaxDamage() - stack.getDamage()) * 100.0f) / stack.getMaxDamage()) + "%", 0, 0, vertexConsumers, new Color(red, green, 0));
                    matrices.pop();
                ...
                if (items.getValue() && enchantments.getValue() && stack.hasEnchantments()) {
                    ...
                    for (Object2IntMap.Entry<RegistryEntry<Enchantment>> entry : Object2IntMaps.fastIterable(enchantments)) {
                        ...
                        matrices.scale(0.5f, 0.5f, 1);
                        UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, str, 0, 0, vertexConsumers, Color.WHITE);
                        matrices.pop();

                        height += UltraByte.FONT_MANAGER.getHeight() / 2;
                    }
                }
            }
            ...
            if (itemName.getValue() && !player.getMainHandStack().isEmpty()) {
                ...
                matrices.push();
                matrices.translate(-UltraByte.FONT_MANAGER.getWidth(itemText) / 2f / 2f, -UltraByte.FONT_MANAGER.getHeight() - 1 - UltraByte.FONT_MANAGER.getHeight() / 2f - 1 - (renderedItems ? (items.getValue() ? 18 + maxEnchants : 1) + (durability.getValue() && renderedDurability ? UltraByte.FONT_MANAGER.getHeight() / 2.0f + 1 : 0) : 0), 0);
                matrices.scale(0.5f, 0.5f, 1);
                UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, itemText, 0, 0, vertexConsumers, Color.WHITE);
                matrices.pop();
            }
