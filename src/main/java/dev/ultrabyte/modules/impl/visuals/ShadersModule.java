...
public class ShadersModule extends Module {
    ...
    public void onRenderShader(RenderShaderEvent event) {
        ...
        UltraByte.SHADER_MANAGER.prepare();
    }
    ...
    public void onRenderEntity(RenderEntityEvent event) {
        ...
        Color color = (event.getEntity() instanceof PlayerEntity player && UltraByte.FRIEND_MANAGER.contains(player.getName().getString()) && !friends.getValue().equals("Sync")) ? (friends.getValue().equals("Default") ? UltraByte.FRIEND_MANAGER.getDefaultFriendColor() : friendColor.getColor()) : this.color.getColor();
        event.setVertexConsumers(UltraByte.SHADER_MANAGER.create(event.getVertexConsumers(), color));
    }
    ...
    public void onRenderEntity$POST(RenderEntityEvent.Post event) {
        ...
        if (players.getValue() || hostiles.getValue() || animals.getValue() || ambient.getValue() || invisibles.getValue() || items.getValue() || crystals.getValue() || others.getValue()) {
            UltraByte.SHADER_MANAGER.getVertexConsumerProvider().draw();
        }

        if (!hands.getValue()) UltraByte.SHADER_MANAGER.render(mode.getValue().equalsIgnoreCase("Fill") ? 0 : mode.getValue().equalsIgnoreCase("Outline") ? 1 : 2, opacity.getValue().intValue() / 255.0f);
    }
    ...
    public void onRenderHand(RenderHandEvent event) {
        ...
        event.setVertexConsumers(UltraByte.SHADER_MANAGER.create(event.getVertexConsumers(), color.getColor()));
    }
    ...
    public void onRenderShader$POST(RenderShaderEvent.Post event) {
        ...
        UltraByte.SHADER_MANAGER.render(mode.getValue().equalsIgnoreCase("Fill") ? 0 : mode.getValue().equalsIgnoreCase("Outline") ? 1 : 2, opacity.getValue().intValue() / 255.0f);
    }
