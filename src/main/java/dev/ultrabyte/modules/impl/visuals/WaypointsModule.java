...
import dev.ultrabyte.managers.WaypointManager;
...
public class WaypointsModule extends Module {
    ...
    public void onPlayerConnect(PlayerConnectEvent event) {
        ...
        synchronized (logoutPoints) {
            ...
            if(waypoint != null) {
                ...
                if(notification.getValue()) UltraByte.CHAT_MANAGER.message(ChatUtils.getPrimary() + waypoint.name + ChatUtils.getSecondary() + " has logged in at " + ChatUtils.getPrimary() + "[" + (int) waypoint.pos.x + ", " + (int) waypoint.pos.y + ", " + (int) waypoint.pos.z  + "]" + ChatUtils.getSecondary() + ".", "waypoints-" + waypoint.name);
            }
        }
    }
    ...
    public void onPlayerDisconnect(PlayerDisconnectEvent event) {
        ...
        synchronized (logoutPoints) {
            ...
            if (player != null) {
                ...
                if(notification.getValue()) UltraByte.CHAT_MANAGER.message(ChatUtils.getPrimary() + waypoint.name + ChatUtils.getSecondary() + " has logged out at " + ChatUtils.getPrimary() + "[" + (int) waypoint.pos.x + ", " + (int) waypoint.pos.y + ", " + (int) waypoint.pos.z  + "]" + ChatUtils.getSecondary() + ".", "waypoints-" + waypoint.name);
            }
        }
    }
    ...
    public void onRenderWorld(RenderWorldEvent event) {
        ...
        if (rally.getValue() && rallyPoint != null && System.currentTimeMillis() - rallyTime <= 120000) {
            if(rallyPoint.getDimension().equals(WorldUtils.getDimension()) && rallyPoint.getServer().equals(UltraByte.SERVER_MANAGER.getServer())) {
        ...
        if(!UltraByte.WAYPOINT_MANAGER.getWaypoints().isEmpty()) {
            for(WaypointManager.Waypoint waypoint : UltraByte.WAYPOINT_MANAGER.getWaypoints().stream().filter(w -> w.getDimension().equals(WorldUtils.getDimension()) && w.getServer().equals(UltraByte.SERVER_MANAGER.getServer())).toList()) {
    ...
    private boolean sameWorld(Waypoint waypoint) {
        ...
        return WorldUtils.getDimension().equals(waypoint.dimension) && UltraByte.SERVER_MANAGER.getServer().equals(waypoint.server);
    }
    ...
    private class Waypoint {
        ...
        public Waypoint(PlayerEntity player) {
            ...
            this.server = UltraByte.SERVER_MANAGER.getServer();
