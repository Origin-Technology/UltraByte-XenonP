...
import dev.ultrabyte.events.impl.UpdateMovementEvent;
...
public class KillAuraModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        Iterable<Entity> entities = mc.world.getOtherEntities(mc.player, PositionUtils.getRadius(mc.player, Math.max(range.getValue().doubleValue(), wallsRange.getValue().doubleValue()) + 1.0), entity ->
                entity.isAlive()
                        ...
                        && (friends.getValue() || !UltraByte.FRIEND_MANAGER.contains(entity.getName().getString()))
        ...
        if (rotate.getValue().equalsIgnoreCase("Hold")) UltraByte.ROTATION_MANAGER.rotate(RotationUtils.getRotations(target), this);
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) UltraByte.ROTATION_MANAGER.rotate(RotationUtils.getRotations(target), this);
    ...
    public void onUpdateMovement$POST(UpdateMovementEvent.Post event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Packet")) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(target));
