...
import dev.ultrabyte.modules.impl.movement.SpeedModule;
import dev.ultrabyte.modules.impl.movement.StepModule;
...
public class SurroundModule extends Module {
    ...
    public void onEnable() {
        ...
        if(stepToggle.getValue() && UltraByte.MODULE_MANAGER.getModule(StepModule.class).isToggled()) UltraByte.MODULE_MANAGER.getModule(StepModule.class).setToggled(false);
        if(speedToggle.getValue() && UltraByte.MODULE_MANAGER.getModule(SpeedModule.class).isToggled()) UltraByte.MODULE_MANAGER.getModule(SpeedModule.class).setToggled(false);
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (mc.player == null || mc.world == null) return;
        if (jumpDisable.getValue() && (mc.player.fallDistance > 2.0f || ((UltraByte.MODULE_MANAGER.getModule(StepModule.class).isToggled() || UltraByte.MODULE_MANAGER.getModule(SpeedModule.class).isToggled()) && (lastPosition == null || lastPosition.getY() != PositionUtils.getFlooredPosition(mc.player).getY())))) {
            setToggled(false);
            return;
        }
        ...
        if (autoSwitch.getValue().equalsIgnoreCase("None") && !(mc.player.getMainHandStack().getItem() instanceof BlockItem)) {
            if (itemDisable.getValue()) {
                UltraByte.CHAT_MANAGER.tagged("You are currently not holding any blocks.", getName());
                setToggled(false);
            }
        ...
        if (slot == -1) {
            if (itemDisable.getValue()) {
                UltraByte.CHAT_MANAGER.tagged("No blocks could be found in your hotbar.", getName());
                setToggled(false);
            }
        ...
        HitboxDesyncModule module = UltraByte.MODULE_MANAGER.getModule(HitboxDesyncModule.class);
