...
public class KillSayModule extends Module {
    private final String[] KILL_MESSAGES = {"Sit [username], open owns me and all!", "[username] I killed you with the power of open!", "Sorry [username], get good get open!", "I just killed [username] thanks to open!"};
    ...
    public void onTick(TickEvent event) {
        ...
        killMessages = FileUtils.readLines(new File(UltraByte.MOD_NAME + "/Client/" + killFile.getValue()));
        popMessages = FileUtils.readLines(new File(UltraByte.MOD_NAME + "/Client/" + popFile.getValue()));

        synchronized (queue) {
            if (timer.hasTimeElapsed(delay.getValue().intValue() * 1000)) {
                String message = queue.poll();
                if(!message.isEmpty()) {
                    if (clientside.getValue()) UltraByte.CHAT_MANAGER.message(message);
    ...
    public void onTargetDeath(TargetDeathEvent event) {
        ...
        if(killStreak.getValue() && streak > 1) {
            UltraByte.CHAT_MANAGER.message("You are on a " + streak + " kill streak.");
        }
    }

    @SubscribeEvent
    public void onPlayerPop(PlayerPopEvent event) {
        if(getNull() || event.getPlayer() == mc.player || !UltraByte.TARGET_MANAGER.isTarget(event.getPlayer()) || !pops.getValue()) return;
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        if(event.getPlayer() == mc.player && (!suicideIgnore.getValue() || !suicide && !UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).isToggled())) {
            streak = 0;
        }
