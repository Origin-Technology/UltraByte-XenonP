...
public class SelfFillModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        if (jumpMode.getValue().equalsIgnoreCase("Packet") && burrow.getValue().equalsIgnoreCase("Bypass")) {
            if (!rotatedBypass) {
                UltraByte.ROTATION_MANAGER.rotate(RotationUtils.getRotations(WorldUtils.getHitVector(lastPosition, direction)), this);
                rotatedBypass = true;
                ticks = 0;
                return;
            }
        ...
        if (autoSwitch.getValue().equalsIgnoreCase("None") && (!(mc.player.getMainHandStack().getItem() instanceof BlockItem) || (obsidianOnly.getValue() && mc.player.getMainHandStack().getItem() != Items.OBSIDIAN))) {
            UltraByte.CHAT_MANAGER.tagged("You are currently not holding any " + (obsidianOnly.getValue() ? "valid " : "") + "blocks.", getName());
            setToggled(false);
            return;
        }
        ...
        if (!autoSwitch.getValue().equalsIgnoreCase("None") && slot == -1) {
            UltraByte.CHAT_MANAGER.tagged("There are currently no " + (obsidianOnly.getValue() ? "valid " : "") + "blocks in your hotbar.", getName());
            setToggled(false);
            return;
        }
    ...
    public void onEnable() {
        ...
        if (!mc.player.isOnGround()) {
            UltraByte.CHAT_MANAGER.tagged("You are currently in the air.", getName());
            setToggled(false);
            return;
        }
