    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        if(manual.getValue()) {
            ...
            if (autoSwitch.getValue().equals("None") && mc.player.getInventory().getMainHandStack().getItem() != Items.BOW) {
                UltraByte.CHAT_MANAGER.tagged("You are currently not holding a bow.", getName());
                setToggled(false);
                return;
            }

            if (!autoSwitch.getValue().equals("None") && slot == -1) {
                UltraByte.CHAT_MANAGER.tagged("Could not find a bow in your hotbar.", getName());
                setToggled(false);
                return;
            }
        ...
        UltraByte.ROTATION_MANAGER.packetRotate(mc.player.getYaw(), -90.0f);
        mc.options.useKey.setPressed(false);
