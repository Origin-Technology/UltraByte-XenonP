...
import dev.ultrabyte.modules.impl.player.SpeedMineModule;
import dev.ultrabyte.modules.impl.player.ThrowXPModule;
...
import static dev.ultrabyte.managers.RotationManager.calculateAngle;
...
public class AutoCrystalModule extends Module {
    ...
    public ModeSetting AttackMode = new ModeSetting("AttackMode", "Automatically attacks crystals that are deemed safe Mode.", new CategorySetting.Visibility(attackCategory), "Hit", new String[]{"Hit", "SyncHit", "Packet", "Normal"});
    ...
    //StrictPlace,SyncRotate,
    public BooleanSetting StrictPlace = new BooleanSetting("StrictPlaceChest", "Strict Direction Chest.", new CategorySetting.Visibility(miscellaneousCategory), false);
    ...
    public NumberSetting PredictTick = new NumberSetting("PredictTick", "Predict Tick.", new CategorySetting.Visibility(predictionCategory), 0, 0, 20);
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        attackedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > UltraByte.SERVER_MANAGER.getPing() * 2L);
        placedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > UltraByte.SERVER_MANAGER.getPing() * 2L + (20 - attackSpeed.getValue().floatValue()) * 50L);
        countedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > UltraByte.SERVER_MANAGER.getPing() * 2L);
        ...
        Runnable runnable = () -> {
            ...
            if (blockDestruction.getValue()) {
                SpeedMineModule module = UltraByte.MODULE_MANAGER.getModule(SpeedMineModule.class);
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        ...
        if ((raytrace.getValue() || crystal.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathHelper.square(attackWallsRange.getValue().doubleValue()))) return;
        float[] angle = calculateAngle(Vec3d.of(crystal.getBlockPos()));
        if (rotate.getValue().equalsIgnoreCase("Packet")) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), angle[0], angle[1], mc.player.isOnGround(),mc.player.horizontalCollision));

        //if (rotate.getValue().equalsIgnoreCase("Packet")) UltraByte.ROTATION_MANAGER.packetRotate(calculateRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)));
        if (rotate.getValue().equalsIgnoreCase("Normal")) UltraByte.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)), UltraByte.ROTATION_MANAGER.getModulePriority(this));
    ...
    public void onDestroyBlock(DestroyBlockEvent event) {
        ...
        if (mineTarget == null || mineTarget.getPosition() == null) {
            UltraByte.RENDER_MANAGER.setRenderPosition(null);
            return;
        }

        BlockPos position = mineTarget.getPosition();
        UltraByte.RENDER_MANAGER.setRenderPosition(position);
        ...
        if ((raytrace.getValue() || mc.player.getEyePos().squaredDistanceTo(Vec3d.ofCenter(position)) > MathHelper.square(placeWallsRange.getValue().doubleValue())))
            return;

        if (rotate.getValue().equalsIgnoreCase("Normal")) UltraByte.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), this, UltraByte.ROTATION_MANAGER.getModulePriority(this) + 1);
        if (!rotate.getValue().equalsIgnoreCase("None")) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
        ...
        for (Entity entity : mc.world.getOtherEntities(null, new Box(position.up())).stream().filter(entity -> entity instanceof EndCrystalEntity).toList()) {
            if (!rotate.getValue().equalsIgnoreCase("None")) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(entity));
    ...
    private void attackCrystals() {
        ...
        for (Entity entity : flag ? placeTarget.obstructions : mc.world.getEntities()) {
            ...
            if ((raytrace.getValue() || crystal.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathHelper.square(attackWallsRange.getValue().doubleValue())))
                continue;
        ...
        if ((raytrace.getValue() || endCrystal.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathHelper.square(attackWallsRange.getValue().doubleValue())))
            return;
        float[] angle = calculateAngle(Vec3d.ofCenter(crystal.getBlockPos()));
        if (rotate.getValue().equalsIgnoreCase("Packet")) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), angle[0], angle[1], mc.player.isOnGround(),mc.player.horizontalCollision));

        //if (rotate.getValue().equalsIgnoreCase("Packet")) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)));
        if (rotate.getValue().equalsIgnoreCase("Normal")) UltraByte.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(crystal.getBlockPos(), 1)), UltraByte.ROTATION_MANAGER.getModulePriority(this));
    ...
    private void placeCrystals(boolean sequential) {
        ...
        if ((raytrace.getValue() || mc.player.getEyePos().squaredDistanceTo(Vec3d.ofCenter(position)) > MathHelper.square(placeWallsRange.getValue().doubleValue()))) return;
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) UltraByte.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), UltraByte.ROTATION_MANAGER.getModulePriority(this));
        ...
        placeRunnable = () -> {
            boolean switched = false;
            float[] angle = calculateAngle(Vec3d.ofCenter(position));
            if  (rotate.getValue().equalsIgnoreCase("Packet")) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), angle[0], angle[1], mc.player.isOnGround(),mc.player.horizontalCollision));
            if (rotate.getValue().equalsIgnoreCase("Normal")) UltraByte.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), UltraByte.ROTATION_MANAGER.getModulePriority(this));

            //if (rotate.getValue().equalsIgnoreCase("Packet")) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
            ...
            if (godSync.getValue()) {
                boolean flag = !antiKick.getValue() || !(mc.player.getMainHandStack().getItem() instanceof ExperienceBottleItem) && !(mc.player.getOffHandStack().getItem() instanceof ExperienceBottleItem) && !UltraByte.MODULE_MANAGER.getModule(ThrowXPModule.class).isToggled();
    ...
    private EndCrystalEntity calculateCrystals() {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (!UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).isToggled()) {
    ...
    private PlaceTarget calculatePlacements(BlockPos exception) {
        ...
        for (int i = 0; i < UltraByte.WORLD_MANAGER.getRadius(Math.max(placeRange.getValue().doubleValue(), placeWallsRange.getValue().doubleValue())); i++) {
            ...
            if ((raytrace.getValue() || mc.player.getEyePos().squaredDistanceTo(Vec3d.ofCenter(position)) > MathHelper.square(placeWallsRange.getValue().doubleValue()))) continue;
    ...
    private float[] calculateRotations(Vec3d vec3d) {
        ...
        if (yawStep.getValue()) {
            ...
            float difference = UltraByte.ROTATION_MANAGER.getServerYaw() - rotations[0];
            ...
            if (Math.abs(difference) > yawStepThreshold.getValue().floatValue()) yaw = UltraByte.ROTATION_MANAGER.getServerYaw() + deltaYaw;
    ...
    private void attack(EndCrystalEntity crystal) {
        ...
        switch (AttackMode.getValue()) {
            case "Normal" -> {
                Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(PlayerInteractEntityC2SPacket.attack(crystal, mc.player.isSneaking()));
            }
            case "Hit" -> {
                mc.player.networkHandler.sendPacket(PlayerInteractEntityC2SPacket.interact(crystal, mc.player.isSneaking(), Hand.MAIN_HAND));
            ...
            case "SyncHit" -> {
                ...
                Objects.requireNonNull(Objects.requireNonNull(mc.getNetworkHandler())).sendPacket(PlayerInteractEntityC2SPacket.attack(crystal, mc.player.isSneaking()));
            }
            case "Packet" -> {
                if (mc.interactionManager != null) {
                    mc.interactionManager.attackEntity(mc.player, crystal);
    ...
    private void place(BlockPos position) {
        ...
        WorldUtils.Place(position,hand,new BlockHitResult(WorldUtils.getHitVector(position, direction), direction, position, false), null,StrictPlace.getValue(),false,0);
    ...
    private List<PlayerEntity> getPlayers() {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            if (UltraByte.FRIEND_MANAGER.contains(player.getName().getString())) continue;
