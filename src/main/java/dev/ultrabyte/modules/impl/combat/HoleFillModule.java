        ...
        Runnable runnable = () -> {
            ...
            if (autoSwitch.getValue().equalsIgnoreCase("None") && flag) {
                if (itemDisable.getValue()) {
                    UltraByte.CHAT_MANAGER.tagged("You are currently not holding any " + (webs.getValue() ? "cobwebs." : "blocks."), getName());
                    setToggled(false);
                }
            ...
            if (slot == -1) {
                if (itemDisable.getValue()) {
                    UltraByte.CHAT_MANAGER.tagged("No " + (webs.getValue() ? "cobwebs" : "blocks") + " could be found in your hotbar.", getName());
                    setToggled(false);
                }
