...
public class AutoTotemModule extends Module {
    ...
    public void onPlayerPop(PlayerPopEvent event) {
        if (event.getPlayer() == mc.player && !UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).isToggled()) {
            ticks = 0;
        }
    }
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        if (item == Items.TOTEM_OF_UNDYING && UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).isToggled() && UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).offhandOverride.getValue()) {
    ...
    private Item getItem() {
        if (useGapple.getValue() && mc.options.useKey.isPressed() && (lethalOverride.getValue() || !needsTotem()) && (mc.player.getMainHandStack().getItem() instanceof SwordItem || mc.player.getMainHandStack().getItem() instanceof AxeItem) && hasItem(Items.GOLDEN_APPLE))
    ...
    private boolean needsTotem() {
        if (UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).isToggled() && UltraByte.MODULE_MANAGER.getModule(SuicideModule.class).offhandOverride.getValue()) return false;
        ...
        return antiMace.getValue() && (mc.world.getPlayers().stream().anyMatch(entity -> entity != mc.player && !UltraByte.FRIEND_MANAGER.contains(entity.getName().getString()) && mc.player.squaredDistanceTo(entity) <= MathHelper.square(maceRange.getValue().floatValue()) && entity.fallDistance >= 1.5 && entity.getMainHandStack().getItem().equals(Items.MACE)));
    }
