...
import dev.ultrabyte.utils.system.ThreadExecutor;
        ...
        Runnable runnable = () -> {
            ...
            if (autoSwitch.getValue().equalsIgnoreCase("None") && mc.player.getMainHandStack().getItem() != Items.COBWEB) {
                if (itemDisable.getValue()) {
                    UltraByte.CHAT_MANAGER.tagged("You are currently not holding any cobwebs.", getName());
                    setToggled(false);
                }
            ...
            if (slot == -1) {
                if (itemDisable.getValue()) {
                    UltraByte.CHAT_MANAGER.tagged("No cobwebs could be found in your hotbar.", getName());
                    setToggled(false);
                }
            ...
            if(rotate.getValue() && airPlace.getValue()) UltraByte.ROTATION_MANAGER.rotate(RotationUtils.getRotations(position.toCenterPos()), this);
