...
import dev.ultrabyte.mixins.accessors.MouseAccessor;
import dev.ultrabyte.modules.Module;
import dev.ultrabyte.modules.impl.movement.HitboxDesyncModule;
import dev.ultrabyte.modules.impl.movement.HoleSnapModule;
...
//@RegisterModule(name = "ultrabyteRobotics", description = "Very cool robots", category = Module.Category.MISCELLANEOUS)
public class ultrabyteRoboticsModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        try {
            ...
            UltraByte.LOGGER.error("An exception has been thrown by clientside ultrabyte Robotics!", exception);
        }
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (UltraByte.MODULE_MANAGER.getModule(HoleSnapModule.class).isToggled()) return;
        if (UltraByte.MODULE_MANAGER.getModule(HitboxDesyncModule.class).isToggled()) return;
    ...
    public void onEnable() {
        if (side.getValue().equalsIgnoreCase("Client")) {
            try {
                ...
                UltraByte.LOGGER.error("An exception has been thrown by clientside ultrabyte Robotics!", exception);
                UltraByte.CHAT_MANAGER.error("Failed to establish a connection to the ultrabyteRobotics server.");
                setToggled(false);
            }
        } else {
            try {
                ...
                UltraByte.LOGGER.error("An exception has been thrown by serverside ultrabyte Robotics!", exception);
                UltraByte.CHAT_MANAGER.error("Failed to start the ultrabyteRobotics server.");
                setToggled(false);
            }
        }
    }
        ...
        public void startConnection(int port) throws IOException {
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                if (split[0].equalsIgnoreCase("module") && split.length == 3) {
                    Module module = UltraByte.MODULE_MANAGER.getModule(split[1]);
                    if (module != null) {
                        UltraByte.TASK_MANAGER.submit(() -> module.setToggled(Boolean.parseBoolean(split[2])));
                    }
                }

                if (split[0].equalsIgnoreCase("update") && split.length == 2) {
                    UltraByte.MODULE_MANAGER.getModule(ultrabyteRoboticsModule.class).setTarget(split[1]);
                }

                if (split[0].equalsIgnoreCase("key") && split.length == 5 && mc.keyboard != null) {
                    UltraByte.TASK_MANAGER.submit(() -> mc.keyboard.onKey(mc.getWindow().getHandle(), Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3]), Integer.parseInt(split[4])));
                }

                if (split[0].equalsIgnoreCase("mouse") && split.length == 4 && mc.mouse != null) {
                    UltraByte.TASK_MANAGER.submit(() -> ((MouseAccessor) mc.mouse).invokeOnMouseButton(mc.getWindow().getHandle(), Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3])));
                }
                ...
                if (!UltraByte.MODULE_MANAGER.getModule(ultrabyteRoboticsModule.class).isToggled()) {
                    stopConnection();
                    break;
                }
            }
