...
import dev.ultrabyte.utils.animations.Animation;
...
public class HUDModule extends Module {
    ...
    public StringSetting watermarkText = new StringSetting("WatermarkText", "Text", "The client name that will be rendered.", new CategorySetting.Visibility(watermarkCategory), UltraByte.MOD_NAME);
    ...
    public BooleanSetting welcomerTypewriter = new BooleanSetting("WelcomerTypewriter","Typewriter", "Enable typewriter effect for the welcomer text.", new CategorySetting.Visibility(welcomerCategory),true);
    public NumberSetting welcomerPauseTime = new NumberSetting("WelcomerPauseTime", "PauseTime","How long to display the full text in milliseconds before hiding.",new CategorySetting.Visibility(welcomerCategory), 1000, 1000, 10000);
    public NumberSetting welcomerSpeed = new NumberSetting("WelcomerSpeed", "Speed", "modifyTheSpeedAtWhichCharactersAreTyping", new CategorySetting.Visibility(welcomerCategory), 200, 50, 500);
    ...
    public BooleanSetting metaData = new BooleanSetting("MetaData", "Data","Whether or not to show module metadata in the module list.", new CategorySetting.Visibility(moduleListCategory), true);
    ...
    public void onEnable() {
        ...
        currentCharIndex = welcomerTypewriter.getValue() ? 0 : fullText.length();
        isTyping = welcomerTypewriter.getValue();
        isPaused = false;
        currentText = welcomerTypewriter.getValue() ? "" : fullText;
    ...
    public void onTick(TickEvent event) {
        ...
        if (moduleList.getValue()) {
            Comparator<Module> widthComparator = Comparator.comparingInt(m -> -UltraByte.FONT_MANAGER.getWidth(getModuleText(m)));
            Comparator<Module> alphabeticalComparator = Comparator.comparing(Module::getName);

            List<ModuleEntry> entries = new ArrayList<>();
            List<Module> modules = UltraByte.MODULE_MANAGER.getModules().stream()
        ...
        if (playerRadar.getValue()) {
            ...
            for (PlayerEntity player : players) {
                ...
                int pops = UltraByte.WORLD_MANAGER.getPoppedTotems().getOrDefault(player.getUuid(), 0);
        ...
        if (potions.getValue()) {
            Comparator<StatusEffectInstance> widthComparator = Comparator.comparingInt(e -> -UltraByte.FONT_MANAGER.getWidth(e.getEffectType().value().getName().getString() + " " + (e.getAmplifier() + 1)));
    ...
    public void renderWatermark(RenderOverlayEvent event) {
        ...
        if (watermark.getValue()) {
            String text = watermarkText.getValue() + (watermarkVersion.getValue() ? (watermarkSync.getValue() ? "" : inversion.getValue() ? Formatting.GRAY : Formatting.WHITE) + " " + UltraByte.MOD_VERSION + (watermarkMinecraftVersion.getValue() ? "-mc" + UltraByte.MINECRAFT_VERSION : "") + (watermarkRevision.getValue() ? "+" + UltraByte.GIT_REVISION + "." + UltraByte.GIT_HASH : "") : "");
            drawText(event.getContext(), text, 2, 2);
        }

        if(uptime.getValue()) {
            String[] hms = FormattingUtils.formatSeconds((System.currentTimeMillis() - UltraByte.UPTIME)/1000);
            drawText(event.getContext(), "Uptime " + Formatting.WHITE + hms[0] + ":" + hms[1] + ":" + hms[2], 2, 2 + (watermark.getValue() ? UltraByte.FONT_MANAGER.getHeight() : 0), informationSync.getValue() ? null : new Color(170, 170, 170));
        }

        if (welcomer.getValue()) {
            String newFullText = welcomerText.getValue().replace("[username]", (welcomerSync.getValue() ? "" : inversion.getValue() ? Formatting.GRAY : Formatting.WHITE) + mc.player.getName().getString() + Formatting.RESET);
            ...
            if (!welcomerTypewriter.getValue()) {
                float x = mc.getWindow().getScaledWidth() / 2.0f - UltraByte.FONT_MANAGER.getWidth(fullText) / 2.0f;
                drawText(event.getContext(), fullText, x, 6);
                return;
            }
            ...
            if (isPaused) {
                long elapsedPauseTime = currentTime - pauseStartTime;
                if (elapsedPauseTime >= welcomerPauseTime.getValue().intValue()) {
                    isPaused = false;
                    isTyping = false;
                } else {
                    float x = mc.getWindow().getScaledWidth() / 2.0f - UltraByte.FONT_MANAGER.getWidth(fullText) / 2.0f;
            ...
            if (currentTime - lastUpdateTime >= welcomerSpeed.getValue().intValue()) {
                lastUpdateTime = currentTime;
                currentCharIndex = Math.min(currentCharIndex, fullText.length());
            ...
            float x = mc.getWindow().getScaledWidth() / 2.0f - UltraByte.FONT_MANAGER.getWidth(currentText) / 2.0f;
            drawText(event.getContext(), currentText, x, 6);
        }
    }
    ...
    public void renderModuleList(RenderOverlayEvent event) {
        ...
        for (ModuleEntry entry : moduleEntries) {
            float x = mc.getWindow().getScaledWidth() - (entry.module().getAnimationOffset().get(entry.module().isToggled() ? UltraByte.FONT_MANAGER.getWidth(entry.text()) + 2 : 0));
            float y = 2 + potionOffset + (index * UltraByte.FONT_MANAGER.getHeight());
    ...
    public void renderPlayerRadar(RenderOverlayEvent event) {
        ...
        for (PlayerEntry entry : playerEntries) {
            if (entry.headTexture() != null) PlayerSkinDrawer.draw(event.getContext(), entry.headTexture(), 2, 1 + (UltraByte.FONT_MANAGER.getHeight() * 2) + ((UltraByte.FONT_MANAGER.getHeight() + 1) * offset), UltraByte.FONT_MANAGER.getHeight(), true, false, Color.WHITE.getRGB());
            drawText(event.getContext(), entry.text(), 2 + (entry.headTexture() != null ? UltraByte.FONT_MANAGER.getHeight() + 2 : 0), 2 + (UltraByte.FONT_MANAGER.getHeight() * 2) + ((UltraByte.FONT_MANAGER.getHeight() + 1) * offset), UltraByte.FRIEND_MANAGER.contains(entry.player().getName().getString()) ? UltraByte.FRIEND_MANAGER.getDefaultFriendColor() : null);
    ...
    public void renderInformation(RenderOverlayEvent event) {
        ...
        if (health.getValue()) {
            ...
            UltraByte.FONT_MANAGER.drawTextWithOutline(event.getContext(), text, mc.getWindow().getScaledWidth() / 2 - UltraByte.FONT_MANAGER.getWidth(text) / 2, mc.getWindow().getScaledHeight() / 2 + 16, Color.WHITE, Color.BLACK);
        }
        ...
        if (potions.getValue()) {
            for (PotionEntry entry : potionEntries) {
                if (entry.sprite() != null) {
                    matrices.push();
                    matrices.translate(mc.getWindow().getScaledWidth() - 2 - UltraByte.FONT_MANAGER.getWidth(entry.text()) - UltraByte.FONT_MANAGER.getHeight() - 2, mc.getWindow().getScaledHeight() - chatOffset - 2 - UltraByte.FONT_MANAGER.getHeight() - (UltraByte.FONT_MANAGER.getHeight() * offset) - 1, 0);
                    event.getContext().drawSpriteStretched(RenderLayer::getGuiTextured, entry.sprite(), 0, 0, UltraByte.FONT_MANAGER.getHeight(), UltraByte.FONT_MANAGER.getHeight());
                    matrices.pop();
                }

                drawText(event.getContext(), entry.text(), mc.getWindow().getScaledWidth() - 2 - UltraByte.FONT_MANAGER.getWidth(entry.text()), mc.getWindow().getScaledHeight() - chatOffset - 2 - UltraByte.FONT_MANAGER.getHeight() - (UltraByte.FONT_MANAGER.getHeight() * offset), potionColor.getValue().equals("Client") && colorMode.getValue().equals("Rainbow") && rainbowMode.getValue().equals("Horizontal"), entry.color());
                offset++;
            }
        }
        ...
        if (ping.getValue()) informationEntries.add(getPrimary() + "Ping " + getSecondary() + UltraByte.SERVER_MANAGER.getPing() + "ms");
        if (fps.getValue()) informationEntries.add(getPrimary() + "FPS " + getSecondary() + UltraByte.RENDER_MANAGER.getFps());
        ...
        if (serverBrand.getValue()) informationEntries.add(getPrimary() + "Brand " + getSecondary() + UltraByte.SERVER_MANAGER.getServerBrand());

        float tickRate = UltraByte.SERVER_MANAGER.getTickRate();
        ...
        if (!informationEntries.isEmpty()) {
            informationEntries.sort(Comparator.comparingInt(UltraByte.FONT_MANAGER::getWidth).reversed());
            for (String text : informationEntries) {
                if (text.startsWith("Durability")) {
                    if (mc.player.getMainHandStack().isDamageable()) {
                        ...
                        drawText(event.getContext(), getPrimary() + "Durability ", mc.getWindow().getScaledWidth() - 2 - UltraByte.FONT_MANAGER.getWidth("Durability ") - UltraByte.FONT_MANAGER.getWidth(s), mc.getWindow().getScaledHeight() - chatOffset - 2 - UltraByte.FONT_MANAGER.getHeight() + (offset * -UltraByte.FONT_MANAGER.getHeight()), informationSync.getValue() ? null : new Color(170, 170, 170));
                        drawText(event.getContext(), s, mc.getWindow().getScaledWidth() - 2 - UltraByte.FONT_MANAGER.getWidth(s), mc.getWindow().getScaledHeight() - chatOffset - 2 - UltraByte.FONT_MANAGER.getHeight() + (offset * -UltraByte.FONT_MANAGER.getHeight()), false, new Color(1.0f - ((maxDamage - damage) / (float) maxDamage), (maxDamage - damage) / (float) maxDamage, 0));
                        offset++;
                    }
                } else {
                    drawText(event.getContext(), text, mc.getWindow().getScaledWidth() - 2 - UltraByte.FONT_MANAGER.getWidth(text), mc.getWindow().getScaledHeight() - chatOffset - 2 - UltraByte.FONT_MANAGER.getHeight() + (offset * -UltraByte.FONT_MANAGER.getHeight()), informationSync.getValue() ? null : new Color(170, 170, 170));
                    offset++;
                }
            }
        }
    ...
    public void renderCoordinates(RenderOverlayEvent event) {
        ...
        if (coordinates.getValue())  {
            ...
            drawText(event.getContext(), text, 2, mc.getWindow().getScaledHeight() - chatOffset - offset - UltraByte.FONT_MANAGER.getHeight() - 2);
            offset += UltraByte.FONT_MANAGER.getHeight();
        }
        ...
        if (direction.getValue()) {
            ...
            drawText(event.getContext(), text, 2, mc.getWindow().getScaledHeight() - chatOffset - offset - UltraByte.FONT_MANAGER.getHeight() - 2, positionSync.getValue() ? null : Color.WHITE);
        }
    }
    ...
    private void drawModuleText(Module module, DrawContext context, String text, float x, float y) {
        ...
        if (moduleColorMode.getValue().equalsIgnoreCase("Rainbow")) {
            long index = ((long) y / UltraByte.FONT_MANAGER.getHeight()) * (rainbowOffset.getValue().longValue() * 10L);
    ...
    public void drawText(DrawContext context, String text, float x, float y, boolean rainbow, Color color) {
        ...
        if (rainbow)  {
            UltraByte.FONT_MANAGER.drawRainbowString(context, text, 0, 0, rainbowOffset.getValue().longValue() * 5L);
        } else {
            UltraByte.FONT_MANAGER.drawTextWithShadow(context, text, 0, 0, color);
        }
    ...
    private Color getHudColor(float offset) {
        if (colorMode.getValue().equalsIgnoreCase("Rainbow")) {
            long index = ((long) offset / UltraByte.FONT_MANAGER.getHeight()) * (rainbowOffset.getValue().longValue() * 10L);
            return ColorUtils.getOffsetRainbow(index);
        } else if (colorMode.getValue().equalsIgnoreCase("Wave")) {
            long index = ((long) offset / UltraByte.FONT_MANAGER.getHeight()) * (rainbowOffset.getValue().longValue() * 20L);
