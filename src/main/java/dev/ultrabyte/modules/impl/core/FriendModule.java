...
import dev.ultrabyte.events.impl.MouseInputEvent;
import dev.ultrabyte.events.impl.SettingChangeEvent;
...
public class FriendModule extends Module {
    ...
    public void onSettingChange(SettingChangeEvent event) {
        if (event.getSetting().equals(friendlyFire)) {
            UltraByte.CHAT_MANAGER.warn(ChatUtils.getPrimary() + "Friendly fire" + ChatUtils.getSecondary() + " has been turned " + (friendlyFire.getValue() ? Formatting.GREEN + "on" + Formatting.RESET + ". Friends will now be attacked." : Formatting.RED + "off" + Formatting.RESET + ". Friends will now no longer be attacked."));
        }
    }
    ...
    public void onMouseInput(MouseInputEvent event) {
        ...
        if (UltraByte.FRIEND_MANAGER.contains(name)) {
            UltraByte.FRIEND_MANAGER.remove(name);
            UltraByte.CHAT_MANAGER.tagged("Successfully removed " + ChatUtils.getPrimary() + name + ChatUtils.getSecondary() + " from your friends list.", "MCF", getName());
        } else {
            UltraByte.FRIEND_MANAGER.add(name);
            UltraByte.CHAT_MANAGER.tagged("Successfully added " + ChatUtils.getPrimary() + name + ChatUtils.getSecondary() + " to your friends list.", "MCF", getName());
        }
    }
}
