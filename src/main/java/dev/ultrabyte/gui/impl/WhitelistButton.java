package dev.ultrabyte.gui.impl;

import dev.ultrabyte.UltraByte;
import lombok.AllArgsConstructor;
import dev.ultrabyte.gui.ClickGuiScreen;
import dev.ultrabyte.gui.api.Button;
import dev.ultrabyte.gui.api.Frame;
import dev.ultrabyte.settings.impl.WhitelistSetting;
...
public class WhitelistButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        UltraByte.FONT_MANAGER.drawTextWithShadow(context, setting.getTag(), getX() + getTextPadding() + 1, getY() + 2, Color.WHITE);
        UltraByte.FONT_MANAGER.drawTextWithShadow(context, Formatting.GRAY + "" + setting.getWhitelist().size(), getX() + getWidth() - getTextPadding() - 1 - UltraByte.FONT_MANAGER.getWidth(setting.getWhitelist().size() + ""), getY() + 2, Color.WHITE);
        ...
        if (open) {
            ...
            if (searching) {
                String cursorChar = UltraByte.CLICK_GUI.isShowLine() ? "|" : " ";
            ...
            UltraByte.FONT_MANAGER.drawTextWithShadow(context, displayedSearch, getX() + getTextPadding() + 2 + offsetX, contentY + (searchBarHeight / 2) - (UltraByte.FONT_MANAGER.getHeight() / 2), searching ? (selecting ? ClickGuiScreen.getButtonColor(getY(), 255) : Color.WHITE) : Color.GRAY);
            ...
            for (int i = 0; i < renderCount; i++) {
                ...
                for (ColoredSegment seg : segments) {
                    textWidth += UltraByte.FONT_MANAGER.getWidth(seg.text);
                }
                ...
                for (ColoredSegment seg : drawSegments) {
                    UltraByte.FONT_MANAGER.drawTextWithShadow(context, seg.text, drawX, itemY + 2, seg.color);
                    drawX += UltraByte.FONT_MANAGER.getWidth(seg.text);
                }
                ...
                UltraByte.FONT_MANAGER.drawTextWithShadow(context, symbol, symbolRenderX, itemY + 2, symbolColor);
            }
        ...
        if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
            ...
            if (keyCode == GLFW.GLFW_KEY_V) {
                try {
                    ...
                    UltraByte.LOGGER.error("{}: Failed to process clipboard paste", e.getClass().getName(), e);
                }
            } else if (keyCode == GLFW.GLFW_KEY_C && selecting) {
                try {
                    mc.keyboard.setClipboard(searchQuery);
                } catch (Exception e) {
                    UltraByte.LOGGER.error("{}: Failed to process clipboard change", e.getClass().getName(), e);
                }
    ...
    private List<ColoredSegment> truncateSegments(List<ColoredSegment> segments, int maxWidth) {
        ...
        for (ColoredSegment seg : segments) {
            int segWidth = UltraByte.FONT_MANAGER.getWidth(seg.text);
            if (currentWidth + segWidth <= maxWidth) {
                ...
                if (spaceLeft > 0) {
                    ...
                    for (int i = 0; i < seg.text.length(); i++) {
                        ...
                        int charWidth = UltraByte.FONT_MANAGER.getWidth(String.valueOf(c));
                        if (charWidth <= spaceLeft - UltraByte.FONT_MANAGER.getWidth("…")) {
                            sb.append(c);
                            spaceLeft -= charWidth;
                        } else {
                            break;
