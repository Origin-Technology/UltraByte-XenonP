...
public class ModeButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        UltraByte.FONT_MANAGER.drawTextWithShadow(context, Formatting.GRAY + setting.getValue(), getX() + getWidth() - getTextPadding() - 1 - UltraByte.FONT_MANAGER.getWidth(setting.getValue()), getY() + 2, Color.WHITE);

        if(open) {
            int i = 0;
            for(String s : setting.getModes()) {
                UltraByte.FONT_MANAGER.drawTextWithShadow(context, (setting.getValue().equals(s) ? "" : Formatting.GRAY) + s, getX() + getTextPadding() + 2, getY() + getParent().getHeight() + i + 2, Color.WHITE);
