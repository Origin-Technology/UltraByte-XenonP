...
public class NumberButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        UltraByte.FONT_MANAGER.drawTextWithShadow(context, listening ? (currentString + (selecting ? "" : UltraByte.CLICK_GUI.isShowLine() ? "|" : "")) : setting.getTag(), getX() + getTextPadding() + 1, getY() + 2, Color.WHITE);
        if(!listening) UltraByte.FONT_MANAGER.drawTextWithShadow(context, Formatting.GRAY + "" + setting.getValue(), getX()+ getWidth() - getTextPadding() - 1 - UltraByte.FONT_MANAGER.getWidth(setting.getValue() + ""), getY() + 2, Color.WHITE);
    }
    ...
    public void keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), GLFW.GLFW_KEY_ENTER)) {
            try {
                ...
                UltraByte.CHAT_MANAGER.warn("Please input a valid " + ChatUtils.getPrimary() + setting.getType().name().toLowerCase() + ChatUtils.getSecondary() + " number.");
            }
            selecting = false;
            listening = false;
            return;
