package dev.ultrabyte.utils.text;

import dev.ultrabyte.UltraByte;
import dev.ultrabyte.mixins.accessors.StyleAccessor;
import dev.ultrabyte.mixins.accessors.TextColorAccessor;
...
public class FormattingUtils {
    ...
    public static List<String> wrapText(String text, int width) {
        ...
        for(String word : words) {
            if(UltraByte.FONT_MANAGER.getWidth(current) + UltraByte.FONT_MANAGER.getWidth(word) <= width) {
                current += word + " ";
            } else {
        ...
        if(UltraByte.FONT_MANAGER.getWidth(current) > 0) wrappedText.add(current);
