package dev.ultrabyte.utils.color;
...
public class ColorUtils {
    ...
    public static Color getGlobalColor() {
        return UltraByte.MODULE_MANAGER.getModule(ColorModule.class).color.getColor();
    }

    public static Color getGlobalColor(int alpha) {
        return getColor(UltraByte.MODULE_MANAGER.getModule(ColorModule.class).color.getColor(), alpha);
    }
    ...
    public static Color getRainbow(int alpha) {
        return getRainbow(UltraByte.MODULE_MANAGER.getModule(ColorModule.class).rainbowSpeed.getValue().longValue(), UltraByte.MODULE_MANAGER.getModule(ColorModule.class).rainbowSaturation.getValue().floatValue() / 100.0f, UltraByte.MODULE_MANAGER.getModule(ColorModule.class).rainbowBrightness.getValue().floatValue() / 100.0f, alpha);
    }
    ...
    public static Color getRainbow(long speed, float saturation, float brightness, int alpha, long index) {
        ...
        Color color = new Color(Color.HSBtoRGB(MathHelper.clamp(hue, 0.0f, 1.0f), MathHelper.clamp(saturation, 0.0f, 1.0f), MathHelper.clamp(brightness, 0.0f, 1.0f)));

        return new Color(color.getRed(), color.getGreen(), color.getBlue(), alpha);
    }

    public static Color getOffsetWave(Color color, long index) {
        return getWave(color, UltraByte.MODULE_MANAGER.getModule(ColorModule.class).rainbowSpeed.getValue().longValue(), 255, index);
    }
    ...
    public static Color getPulse(Color color, long speed) {
        ...
        double sin = Math.sin(2 * Math.PI * (speed/20f) * ((System.currentTimeMillis() - UltraByte.UPTIME)/1000f));
