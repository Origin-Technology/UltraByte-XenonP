...
import dev.ultrabyte.modules.impl.miscellaneous.NameProtectModule;
...
public class FontRenderer implements Closeable, IMinecraft {
    ...
    public void drawText(MatrixStack matrices, OrderedText text, float x, float y, int color, boolean dropShadow) {
        ...
        if (UltraByte.MODULE_MANAGER != null && UltraByte.MODULE_MANAGER.getModule(FontModule.class).isToggled()) {
            x += UltraByte.MODULE_MANAGER.getModule(FontModule.class).xOffset.getValue().floatValue();
            y += UltraByte.MODULE_MANAGER.getModule(FontModule.class).yOffset.getValue().floatValue();
        }
