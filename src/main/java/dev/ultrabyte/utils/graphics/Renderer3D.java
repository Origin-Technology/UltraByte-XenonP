...
import dev.ultrabyte.mixins.accessors.WorldRendererAccessor;
...
public class Renderer3D implements IMinecraft {
    ...
    public static void renderText(MatrixStack matrices, String text, double x, double y, double z, float scaling, boolean background, Color color) {
        ...
        if (background) Renderer2D.renderQuad(matrices, -UltraByte.FONT_MANAGER.getWidth(text) / 2.0f - 2, -UltraByte.FONT_MANAGER.getHeight() - 2, UltraByte.FONT_MANAGER.getWidth(text) / 2.0f + 2, 1, new Color(0, 0, 0, 100));
        UltraByte.FONT_MANAGER.drawTextWithShadow(matrices, text, -UltraByte.FONT_MANAGER.getWidth(text) / 2, -UltraByte.FONT_MANAGER.getHeight(), vertexConsumers, color);

        matrices.pop();
    }

    public static void renderText(MatrixStack matrices, String text, Box box, float scaling, boolean background, Color color) {
        VertexConsumerProvider.Immediate vertexConsumers = mc.getBufferBuilders().getEntityVertexConsumers();

        Vec3d vec3d = new Vec3d(box.minX - mc.getEntityRenderDispatcher().camera.getPos().x, box.minY - mc.getEntityRenderDispatcher().camera.getPos().y, box.minZ - mc.getEntityRenderDispatcher().camera.getPos().z);
