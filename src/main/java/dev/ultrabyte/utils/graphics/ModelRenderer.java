package dev.ultrabyte.utils.graphics;
...
import dev.ultrabyte.utils.mixins.IMultiPhaseParameters;
...
public class ModelRenderer implements IMinecraft {
    ...
    public static void renderModel(Entity entity, boolean staticEntity, float scale, float tickDelta, Render render) {
        ...
        if (!UltraByte.MODULE_MANAGER.getModule(NoInterpolationModule.class).isToggled()) ModelRenderer.offset = new Vec3d(MathHelper.lerp(tickDelta, entity.lastRenderX, entity.getX()), MathHelper.lerp(tickDelta, entity.lastRenderY, entity.getY()), MathHelper.lerp(tickDelta, entity.lastRenderZ, entity.getZ()));
        ...
        if (renderer instanceof LivingEntityRenderer<?, ?, ?> && renderState instanceof LivingEntityRenderState state) {
            ((ILivingEntityRenderer) renderer).ultrabyte$render(state, matrices, CustomVertexConsumerProvider.INSTANCE, 15);
        }
    ...
    private static class CustomVertexConsumerProvider implements VertexConsumerProvider {
        ...
        public VertexConsumer getBuffer(RenderLayer layer) {
            if (layer instanceof IMultiPhase phase && ((IMultiPhaseParameters) (Object) phase.ultrabyte$getParameters()).ultrabyte$getTarget() == RenderLayer.ITEM_ENTITY_TARGET) {
