...
public class WorldUtils implements IMinecraft {
    ...
    public static void Place(BlockPos position, Hand hand, BlockHitResult blockHitResult, Runnable runnable, boolean strict, boolean rotate, int sequence){
        ...
        if (mc.world != null) {
            sneak = WorldUtils.RIGHT_CLICKABLE_BLOCKS.contains(mc.world.getBlockState(position).getBlock()) && !mc.player.isSneaking();
        }
        ...
        if (rotate) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));

        Direction direction = WorldUtils.getClosestDirection(position, strict);
        Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new PlayerInteractBlockC2SPacket(hand, new BlockHitResult(WorldUtils.getHitVector(position, direction), direction, position, false), 0));
    ...
    public static void placeBlock(BlockPos position, Direction direction, Hand hand, Runnable runnable, boolean rotate, boolean crystalDestruction, boolean render) {
        ...
        if (rotate) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(vec3d.getX(), vec3d.getY(), vec3d.getZ()));
     ...
     //   if (sprint) mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.STOP_SPRINTING));
      ...
      //  Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new PlayerInteractBlockC2SPacket(hand, new BlockHitResult(WorldUtils.getHitVector(position, direction), direction, position, false), 0));
        ...
        UltraByte.WORLD_MANAGER.getPlaceTimer().reset();
