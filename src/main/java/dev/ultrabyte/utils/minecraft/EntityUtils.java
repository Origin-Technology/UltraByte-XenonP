...
public class EntityUtils implements IMinecraft {
    ...
    public static boolean isBot(PlayerEntity player) {
        if (UltraByte.MODULE_MANAGER.getModule(FakePlayerModule.class).isToggled() && player == UltraByte.MODULE_MANAGER.getModule(FakePlayerModule.class).getPlayer()) {
            return false;
        }
    ...
    public static double getSpeed(Entity entity, SpeedUnit unit) {
        ...
        if (unit == SpeedUnit.KILOMETERS) return (speed * 3.6 * UltraByte.WORLD_MANAGER.getTimerMultiplier()) * 20;
        else return speed / 0.05 * UltraByte.WORLD_MANAGER.getTimerMultiplier();
    }
