...
public class ModulesCommand extends Command {
    ...
    public void execute(String[] args) {
        if (args.length == 0) {
            List<Module> modules = UltraByte.MODULE_MANAGER.getModules();

            if (modules.isEmpty()) {
                UltraByte.CHAT_MANAGER.tagged("There are currently no registered modules.", getTag(), getName());
            } else {
                ...
                UltraByte.CHAT_MANAGER.message("Modules " + ChatUtils.getPrimary() + "[" + ChatUtils.getSecondary() + modules.size() + ChatUtils.getPrimary() + "]: " + ChatUtils.getSecondary() + builder, getName());
            }
        } else {
            messageSyntax();
        }
