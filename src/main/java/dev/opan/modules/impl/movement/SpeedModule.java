...
import dev.opan.mixins.accessors.Vec3dAccessor;
...
public class SpeedModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        if (mode.getValue().equalsIgnoreCase("Strafe") || mode.getValue().equalsIgnoreCase("StrafeStrict")) {
            ...
            UltraByte.WORLD_MANAGER.setTimerMultiplier(useTimer.getValue() && flag && (ticks > bypassThreshold.getValue().intValue() || !timerBypass.getValue()) ? timerMultiplier.getValue().floatValue() : 1.0f);
        }
        ...
        if (mode.getValue().equalsIgnoreCase("Grim")) {
            ...
            for (Entity entity : mc.world.getEntities()) {
                if (entity != null && entity != mc.player && entity instanceof LivingEntity && !(UltraByte.MODULE_MANAGER.getModule(FakePlayerModule.class).isToggled() && UltraByte.MODULE_MANAGER.getModule(FakePlayerModule.class).getPlayer() == entity) && !(entity instanceof ArmorStandEntity) && MathHelper.sqrt((float) mc.player.squaredDistanceTo(entity)) <= 1.5) {
                    collisions++;
                }
            }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        if (mode.getValue().equalsIgnoreCase("Strafe") || mode.getValue().equalsIgnoreCase("StrafeStrict")) {
            if ((UltraByte.MODULE_MANAGER.getModule(HoleSnapModule.class).isToggled() && UltraByte.MODULE_MANAGER.getModule(HoleSnapModule.class).hole != null)) return;
