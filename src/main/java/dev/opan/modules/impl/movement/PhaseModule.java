    ...
    public void onEnable() {
        ...
        if (mode.getValue().equalsIgnoreCase("Pearl")) {
            ...
            if (autoSwitch.getValue().equalsIgnoreCase("None") && mc.player.getMainHandStack().getItem() != Items.ENDER_PEARL) {
                UltraByte.CHAT_MANAGER.tagged("You are currently not holding any pearls.", getName());
                setToggled(false);
                return;
            }
            ...
            if (slot == -1) {
                UltraByte.CHAT_MANAGER.tagged("No pearls could be found in your hotbar.", getName());
                setToggled(false);
                return;
            }
            ...
            if (fireCharge.getValue() && mc.world.getBlockState(mc.player.getBlockPos()).isAir() && !(mc.world.getBlockState(downPosition).isReplaceable())) {
                ...
                if (chargeSlot != -1) {
                    UltraByte.ROTATION_MANAGER.packetRotate(yaw, 90);
            ...
            UltraByte.ROTATION_MANAGER.packetRotate(yaw, pitch.getValue().intValue());
