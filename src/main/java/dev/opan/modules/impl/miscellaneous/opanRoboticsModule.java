...
import dev.opan.mixins.accessors.MouseAccessor;
import dev.opan.modules.Module;
import dev.opan.modules.impl.movement.HitboxDesyncModule;
import dev.opan.modules.impl.movement.HoleSnapModule;
...
//@RegisterModule(name = "opanRobotics", description = "Very cool robots", category = Module.Category.MISCELLANEOUS)
public class opanRoboticsModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        try {
            ...
            UltraByte.LOGGER.error("An exception has been thrown by clientside opan Robotics!", exception);
        }
    }
    ...
    public void onEnable() {
        if (side.getValue().equalsIgnoreCase("Client")) {
            try {
                ...
                UltraByte.LOGGER.error("An exception has been thrown by clientside opan Robotics!", exception);
                UltraByte.CHAT_MANAGER.error("Failed to establish a connection to the opanRobotics server.");
                setToggled(false);
            }
        } else {
            try {
                ...
                UltraByte.LOGGER.error("An exception has been thrown by serverside opan Robotics!", exception);
                UltraByte.CHAT_MANAGER.error("Failed to start the opanRobotics server.");
                setToggled(false);
            }
        }
    }
        ...
        public void startConnection(int port) throws IOException {
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                if (split[0].equalsIgnoreCase("update") && split.length == 2) {
                    UltraByte.MODULE_MANAGER.getModule(opanRoboticsModule.class).setTarget(split[1]);
                }
                ...
                if (!UltraByte.MODULE_MANAGER.getModule(opanRoboticsModule.class).isToggled()) {
                    stopConnection();
                    break;
                }
            }
