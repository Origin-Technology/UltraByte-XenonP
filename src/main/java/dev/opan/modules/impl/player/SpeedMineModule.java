        ...
        if (doubleMine.getValue()) {
            ...
            if (target != null) {
                ...
                if (sequence.getValue().equals("Surround")) {
                    ...
                    inside.run();
                    outside.run();
                }else if (sequence.getValue().equals("PistonCrystal")) {
                    inside.run();
                    outside.run();
                }  else if (sequence.getValue().equals("AutoCrystal")) {
                inside.run();
                outside.run();
            }
            }
    ...
    public class Action {
        ...
        public boolean process() {
            ...
            if (mining) {
                ...
                float delta = WorldUtils.getMineSpeed(this.state, slot) / UltraByte.WORLD_MANAGER.getTimerMultiplier();
                ...
                if (rotate.getValue().equalsIgnoreCase("Normal") && progress + (delta * 2) >= getSpeed()) {
                    UltraByte.ROTATION_MANAGER.rotate(RotationUtils.getRotations(WorldUtils.getHitVector(position, direction)), UltraByte.ROTATION_MANAGER.getModulePriority(UltraByte.MODULE_MANAGER.getModule(SpeedMineModule.class)));
                }
                ...
                if (progress >= getSpeed() && !state.isReplaceable() && (whileEating.getValue() || !mc.player.isUsingItem())) {
                    if (!instantMine || instantTimer.hasTimeElapsed(instantDelay.getValue().longValue() * 50L)) {
                        UltraByte.EVENT_HANDLER.post(new DestroyBlockEvent(position));

                        if (rotate.getValue().equalsIgnoreCase("Packet")) UltraByte.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(WorldUtils.getHitVector(position, direction)));
