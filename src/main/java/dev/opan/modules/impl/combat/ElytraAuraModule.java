...
import dev.opan.events.impl.KeyInputEvent;
...
@RegisterModule(name = "ElytraAuraModule", description = "Automatically equips the best armor.", category = Module.Category.MISCELLANEOUS)
public class ElytraAuraModule extends Module {
    public ModeSetting health = new ModeSetting("Health", "The health priority to apply.", "Highest", new String[]{"Highest", "Lowest", "Any"});
    public BooleanSetting Priority = new BooleanSetting("Priority", "Prioritizes elytra over armor pieces.", true);
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        if(smartElytra.getValue() && elytra.getValue()) {
            if((mc.player.isOnGround() && !(mc.player.getMainHandStack().getItem() instanceof ExperienceBottleItem) || EntityUtils.isInWeb(mc.player) || UltraByte.MODULE_MANAGER.getModule(MaceAuraModule.class).isToggled())) elytra.setValue(false);
        }
    ...
    private void update(EquipmentSlot type, int x) {
        int elytraSlot = findElytra(type);
    ...
    private int findElytra(EquipmentSlot type) {
        int slot = -1;
        for (int i = InventoryUtils.HOTBAR_START; i <= InventoryUtils.INVENTORY_END; i++) {
            ItemStack stack = mc.player.getInventory().getStack(i);
            if (!(stack.getItem() instanceof ArmorItem)) continue;
