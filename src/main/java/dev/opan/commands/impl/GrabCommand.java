...
public class GrabCommand extends Command {
    ...
    public void execute(String[] args) {
        if (args.length == 1) {
            switch (args[0]) {
                case "ip" -> copy(UltraByte.SERVER_MANAGER.getServer());
    ...
    mc.keyboard.setClipboard(text);
        UltraByte.CHAT_MANAGER.tagged("Successfully copied " + ChatUtils.getPrimary() + text + ChatUtils.getSecondary() + " to your clipboard.", getTag(), getName());
    }
}
