package dev.opan.gui.api;

import dev.opan.UltraByte;
import lombok.Getter;
import lombok.Setter;
import dev.opan.gui.ClickGuiScreen;
import dev.opan.gui.impl.WhitelistButton;
import dev.opan.modules.Module;
import dev.opan.gui.impl.ModuleButton;
import dev.opan.modules.impl.core.ClickGuiModule;
...
public class Frame {
    ...
    public Frame(Module.Category category, int x, int y, int width, int height) {
        ...
        for(Module module : UltraByte.MODULE_MANAGER.getModules(category)) buttons.add(new ModuleButton(module, this, height));
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        UltraByte.FONT_MANAGER.drawTextWithShadow(context, category.getName(), x + textPadding, y + 2, Color.WHITE);

        if(open) {
            Color color = UltraByte.MODULE_MANAGER.getModule(ClickGuiModule.class).color.getColor();
            Renderer2D.renderQuad(context.getMatrices(), x, y + height, x + width, y + totalHeight + 1, UltraByte.MODULE_MANAGER.getModule(ClickGuiModule.class).isRainbow() ? new Color(0, 0, 0, 100) : new Color((int) (color.getRed()*0.3), (int) (color.getGreen()*0.3), (int) (color.getBlue()*0.3), 100));
            for(Button button : buttons) {
    ...
    public void mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (x <= mouseX && x + width > mouseX) {
            ...
            if (!whitelistHandling) {
                if (verticalAmount < 0) {
                    setY(getY() - UltraByte.MODULE_MANAGER.getModule(ClickGuiModule.class).scrollSpeed.getValue().intValue());
